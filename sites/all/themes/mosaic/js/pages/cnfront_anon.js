...
// Document loaded!
(function($) {
  Drupal.behaviors.mosaicCNFrontAnonInit = {
    attach : function(context, settings) {
      try {
        if (context.nodeName === '#document') {
          new Drupal.mosaic.mosaicCNAnonFront();
        }
      }
      catch(err) {
        console.log('mosaicCNFrontAnonInit errored: '+err);
      }
    }
  };
  ...
  // Set it off!
  Drupal.mosaic.mosaicCNAnonFront = function() {
    try {
      ...
      console.log('mosaicCNAnonFront errored: '+err);
    }
  };


  // Prepare the data
  Drupal.mosaic.mosaicCNAnonFront.prototype.init = function() {
    try {
      this.speed     = 500;
  ...
  Drupal.mosaic.mosaicCNAnonFront.prototype.prime = function() {
    try {
      this.update();
    }
    catch (err) {
  ...
  // Set events
  Drupal.mosaic.mosaicCNAnonFront.prototype.events = function() {
    try {
      var CNF = this;
  ...
  Drupal.mosaic.mosaicCNAnonFront.prototype.update = function() {
    try {
